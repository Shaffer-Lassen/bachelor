...
class ScratchDecoder(nn.Module):
    def __init__(self, base_channels=64):
        super().__init__()
        self.up_conv3 = nn.Conv2d(base_channels*8 + base_channels*4, base_channels*4, 3, padding=1)
        self.up_bn3   = nn.BatchNorm2d(base_channels*4)
        self.up_conv2 = nn.Conv2d(base_channels*4 + base_channels*2, base_channels*2, 3, padding=1)
        self.up_bn2   = nn.BatchNorm2d(base_channels*2)
        self.up_conv1 = nn.Conv2d(base_channels*2 + base_channels, base_channels, 3, padding=1)
        self.up_bn1   = nn.BatchNorm2d(base_channels)
        self.out_conv = nn.Conv2d(base_channels, 1, kernel_size=3, stride=1, padding=1)

    def forward(self, feats):
        x0, x1, x2, x3, x4 = feats
        y3 = F.interpolate(x4, size=(x3.size(2), x3.size(3)), mode='bilinear', align_corners=False)
        y3 = torch.cat([y3, x3], dim=1)
        y3 = F.relu(self.up_bn3(self.up_conv3(y3)), inplace=True)
        y2 = F.interpolate(y3, size=(x2.size(2), x2.size(3)), mode='bilinear', align_corners=False)
        y2 = torch.cat([y2, x2], dim=1)
        y2 = F.relu(self.up_bn2(self.up_conv2(y2)), inplace=True)
        y1 = F.interpolate(y2, size=(x1.size(2), x1.size(3)), mode='bilinear', align_corners=False)
        y1 = torch.cat([y1, x1], dim=1)
        y1 = F.relu(self.up_bn1(self.up_conv1(y1)), inplace=True)
...
class ScratchModel(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, num_heads=4):
        ...
        self.decoder = ScratchDecoder(base_channels=base_channels)
...
if __name__ == "__main__":
    model = ScratchModel(in_channels=3, base_channels=64, num_heads=4)
